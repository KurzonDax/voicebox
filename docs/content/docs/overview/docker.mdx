...
This fork does not publish prebuilt Docker images. Use the Dockerfile included in the repository to build locally:
...
# Run it
docker run -p 17493:17493 -v voicebox-data:/app/data voicebox
```

Or simply use `docker compose up` as described above — it builds the image from the repo's Dockerfile automatically.

The CPU image is ~3-4 GB (Python + PyTorch + TTS packages). For NVIDIA GPU support, add the GPU device configuration shown in the GPU Acceleration section above.
