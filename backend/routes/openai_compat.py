...
class SpeechRequest(BaseModel):
    ...
    Fields mirror the OpenAI ``/v1/audio/speech`` endpoint so the official
    SDK can serialize requests directly.
    """
...
async def create_speech(
    ...
    # --- Fail fast if model not cached, then load ----------------------
...
async def list_models() -> dict:
    ...
    Response shape mirrors OpenAI's ``GET /v1/models`` so SDK clients can
    enumerate available models without code changes.
    """
    return {
        "object": "list",
        "data": [{"id": model_id, "object": "model", "owned_by": "voicebox"} for model_id in _AVAILABLE_MODELS],
    }
...
async def _resolve_voice_prompt(voice: str, engine: str, db: Session) -> dict:
    ...
    profile = db.query(DBVoiceProfile).filter(func.lower(DBVoiceProfile.name) == voice.lower()).first()
    ...
    if profile is not None:
        try:
            ...
            logger.warning(
                "Failed to create voice prompt for profile '%s': %s. Falling back to built-in Kokoro voice.",
                voice,
                e,
            )
